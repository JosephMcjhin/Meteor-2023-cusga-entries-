...
public class InventoryManager : MonoBehaviour
{
    ...
    public static void SwapList(int x1,int from1,int x2,int from2){
        ...
        if((from1 == 2 && x1 == 0) || (from2 == 2 && x2 == 0)){     //武器栏变化时，将武器的对象创建或者销毁
            if(Player.instance.Weapon_slot.transform.childCount != 0){
                //Debug.Log(Player.instance.Weapon_slot.transform.childCount);
                Destroy(Player.instance.Weapon_slot.transform.GetChild(0).gameObject);
            }
            Player.instance.Weapon = instance.Bag[1].ItemList[0].Weapon;
            if(instance.Bag[1].ItemList[0].Weapon != null){
                GameObject new_Weapon = Instantiate(instance.Bag[1].ItemList[0].Weapon,Player.instance.Weapon_slot.transform.position,Quaternion.identity);
                new_Weapon.transform.SetParent(Player.instance.Weapon_slot.transform);
            }
        }
